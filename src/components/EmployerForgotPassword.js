...
const ForgotPassword = () => {
  ...
  const handleSendOTP = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:8080/verifyEmployerEmail`, { email });
...
if (!validatePassword(password)) {
  ...
  return;
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:8080/reset-Employer-password`, {
        email,
        password,
